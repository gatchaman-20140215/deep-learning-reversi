...
std::uint16_t montecarlo_ai(
        const std::uint16_t black, const std::uint16_t white, const Color currentColor, int trials, bool debug) {
    ...
    ai->debug(debug);
	uint16_t best = ai->act(board);
	delete ai;
	ai = NULL;
	return best;
}

std::uint16_t montecarlo_tree_ai(const std::uint16_t black, const std::uint16_t white,
...
std::uint16_t negamax_ai(const std::uint16_t black, const std::uint16_t white, const Color currentColor, bool debug) {
	Board board(black, white, currentColor);
    if (debug)
	   std::cout << board.str() << std::endl;
...
std::uint16_t random_ai(const std::uint16_t black, const std::uint16_t white, const Color currentColor, bool debug) {
	Board board(black, white, currentColor);
    if (debug)
    	std::cout << board.str() << std::endl;
...
int evaluate(const std::uint16_t black, const std::uint16_t white, const Color currentColor, bool debug) {
	Board board(black, white, currentColor);
    if (debug)
	   std::cout << board.str() << std::endl;
...
PYBIND11_MODULE(ReversiWrapper, m) {
	...
	m.def("negamax_ai", &negamax_ai, py::arg("black"), py::arg("white"), py::arg("current_color"), py::arg("debug"));
